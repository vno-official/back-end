...
public class AuthController {
  ...
  public ResponseEntity<com.vno.common.dto.ApiResponse<Void>> logoutAll(
      @Parameter(hidden = true) Authentication authentication) {
    UUID userId = UUID.fromString(authentication.getName());
    authService.logoutAll(userId);
    return ResponseEntity.ok(com.vno.common.dto.ApiResponse.success("All sessions logged out successfully", null));
  }

  @GetMapping("/sessions")
  @Operation(summary = "Get active sessions", description = "Returns all active sessions for the user")
  @SecurityRequirement(name = "bearerAuth")
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "Sessions retrieved successfully"),
      @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "Unauthorized")
  })
  public ResponseEntity<com.vno.common.dto.ApiResponse<List<UserSessionResponse>>> getSessions(
      @Parameter(hidden = true) Authentication authentication) {
    UUID userId = UUID.fromString(authentication.getName());
    List<UserSessionResponse> sessions = authService.getUserSessions(userId);
    return ResponseEntity.ok(com.vno.common.dto.ApiResponse.success(sessions));
  }
  ...
  public ResponseEntity<com.vno.common.dto.ApiResponse<Void>> revokeSession(
      @Parameter(description = "Session ID to revoke", required = true) @PathVariable UUID sessionId,
      @Parameter(hidden = true) Authentication authentication) {
    UUID userId = UUID.fromString(authentication.getName());
    authService.revokeSession(sessionId, userId);
    return ResponseEntity.ok(com.vno.common.dto.ApiResponse.success("Session revoked successfully", null));
  }
}